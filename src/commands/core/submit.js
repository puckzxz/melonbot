...
module.exports = class SubmitCommand extends Command {
    ...
    async run(msg, args) {
        ...
        const embed = new RichEmbed()
            ...
            .addField("Realeased", movieData.Year, true)
            .setColor(0x00FF00);
...
function getMovieData(imdbID) {
    xmlHttp.open("GET", `http://www.omdbapi.com/?i=${imdbID}&apikey=${process.env.API_KEY}`, false);
    xmlHttp.send(null);
    let respJSON = JSON.parse(xmlHttp.responseText);
    let genreList = respJSON.Genre.split(', ');
    let rating = respJSON.imdbRating;
    return {
...
function addMovieToDB(movieDetailsJSON) {
    MongoClient.connect(mongoURL, {
        ...
        dbo.collection(process.env.MONGO_COL).insertOne(movie, function (err, res) {
            if (err) {
                return console.log(chalk.red(err));
            }
